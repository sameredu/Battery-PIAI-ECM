...
pd.DataFrame(sens).to_csv(f"{TABLES}/b0007_eol_sensitivity.csv",index=False)
...
main.round(3).to_csv(f"{TABLES}/lobo_folds.csv",index=False)
...
ablt.to_csv(f"{TABLES}/ablation_baselines.csv",index=False)
R['ablation']=ablt.to_dict('records')
print("\n### ABLATION ###"); print(ablt.to_string(index=False))

json.dump(R, open(os.path.join(ROOT,"results","results_summary.json"),"w"), indent=2)
print("\nsaved")
